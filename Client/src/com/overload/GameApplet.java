...
public class GameApplet extends Applet implements Runnable, MouseListener, MouseMotionListener, MouseWheelListener,
    ...
    final void createClientFrame(int w, int h) {
        ...
        myWidth = w;
        myHeight = h;
        gameFrame = new GameFrame(this, myWidth, myHeight, Client.frameMode == ScreenMode.RESIZABLE,
                Client.frameMode == ScreenMode.FULLSCREEN);
    ...
    public void mouseWheelMoved(MouseWheelEvent event) {
        ...
        if (mouseX > 0 && mouseX < 512 && mouseY > Client.frameHeight - 165 && mouseY < Client.frameHeight - 25) {
            ...
            if (scrollPos < 0)
                scrollPos = 0;
            if (scrollPos > Client.scrollBarPosition - 110)
                scrollPos = Client.scrollBarPosition - 110;
            if (Client.anInt1089 != scrollPos) {
                Client.anInt1089 = scrollPos;
                Client.instance.updateChatbox = true;
            }
        } else if (Client.loggedIn) {

            /** ZOOMING **/
            boolean zoom = Client.frameMode == ScreenMode.FIXED ? (mouseX < 512) : (mouseX < Client.frameWidth - 200);
            if (zoom &&  Client.instance.openInterfaceId == -1) {
    ...
    public void handleInterfaceScrolling(MouseWheelEvent event) {
        ...
        if (Client.instance.openInterfaceId != -1) {
			int w = 512, h = 334;
            ...
            Widget rsi = Widget.interfaceCache[Client.instance.openInterfaceId];
            if (rsi != null) {
                offsetX = Client.frameMode == ScreenMode.FIXED ? 4 : x;
                offsetY = Client.frameMode == ScreenMode.FIXED ? 4 : y;
    ...
    public final void keyPressed(KeyEvent keyevent) {
        ...
        if (keyevent.isControlDown() && i == KeyEvent.VK_V) {
            Client.instance.inputString = getClipboardContents();
        }
        
        if (i == KeyEvent.VK_ENTER
				&& Client.instance.inputString.length() <= 0) {
        ...
        if (i == KeyEvent.VK_ESCAPE) {
        	...
        	} else if(Configuration.escapeCloseInterface) {
        		if (Client.loggedIn && Client.instance.openInterfaceId != -1) {
                    Client.instance.sendPacket(new CloseInterface().create());
                    return;
                }
        	}
