...
public final class ItemDefinition {
    ...
    public static void init(FileArchive archive) {
        ...
        TOTAL_ITEMS = stream.readUShort() + 21;
        streamIndices = new int[TOTAL_ITEMS + 50000];
        untradeable = new boolean[TOTAL_ITEMS];
        int offset = 2;

        for (int _ctr = 0; _ctr < TOTAL_ITEMS - 21; _ctr++) {
        ...
        System.err.println("[Overload] Loaded: " + TOTAL_ITEMS + " items");
    }
    ...
    public static Sprite getSprite(int itemId, int stackSize, int outlineColor) {
    	return getSprite(itemId, stackSize, outlineColor, false);
    }

    public static Sprite getSprite(int itemId, int stackSize, int outlineColor, boolean forceBlockNote) {
		ItemDefinition itemDef = lookup(itemId);
    	if (forceBlockNote) {
			itemId = itemDef.unnoted_item_id;
			if (itemId == -1)
				itemId = itemDef.noted_item_id;
    	}
		if (outlineColor == 0) {
    ...
    public static Sprite getSprite(int itemId, long stackSize, int outlineColor) {
		if (outlineColor == 0) {
			Sprite sprite = (Sprite) sprites.get(itemId);
			if (sprite != null && sprite.maxHeight != stackSize
					&& sprite.maxHeight != -1) {
		...
		if (itemDef.stackable)
			...
			enabledSprite.maxWidth = 32;
		enabledSprite.maxHeight = (int)stackSize;
		return enabledSprite;
	}
    ...
    public static boolean[] untradeable;
	...
	public void readValues(Buffer buffer) {
		do {
			...
			if (opCode == 1)
				...
				if (actions[opCode - 35].equalsIgnoreCase("null")) {
					actions[opCode - 35] = null;
				} else if (actions[opCode - 35].equalsIgnoreCase("destroy")
						|| actions[opCode - 35].equalsIgnoreCase("bind")
						|| actions[opCode - 35].equalsIgnoreCase("discard")) {
					untradeable[id] = true;
				}
			} else if (opCode == 40) {
