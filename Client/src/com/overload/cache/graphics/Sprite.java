...
import com.overload.util.FileUtils;
...
public class Sprite extends Rasterizer2D {
    ...
    public static Image downScaleImage(Image image, int width, int height) {
		if(width > 765 || width > 503)
			return image.getScaledInstance(width, height, Image.SCALE_SMOOTH);
		else
			return image;
	}
    
    public Sprite(URL url, int scaleX,int scaleY) {		
    	try {
			Image image = ImageIO.read(url.openStream());
			Image resize = downScaleImage(image, scaleX, scaleY);
    ...
    public Sprite(URL url) {
        try {
        	...
        	BufferedImage image = getScaledImage(image1,60,60);
    ...
    public Sprite(BufferedImage bufferedImage, int width, int height) {
		try {
    ...
    public Sprite resizeSprite(int desiredWidth, int desiredHeight) {
		...
		return new Sprite(scaleImage(image, desiredWidth, desiredHeight));
	}
    ...
    public static Image combineIcons(ImageIcon i1, ImageIcon i2, int offsetX,
			int offsetY) {
		Image image1 = i1.getImage();
		Image image2 = i2.getImage();
		int w = Math.max(i1.getIconWidth(), offsetX + i2.getIconWidth());
		int h = Math.max(i1.getIconHeight(), offsetY + i2.getIconHeight());
		Image image = new BufferedImage(w, h, BufferedImage.TYPE_INT_ARGB);
		Graphics g = image.getGraphics();
		g.drawImage(image1, 0, 0, null);
		g.drawImage(image2, offsetX, offsetY, null);
		g.dispose();
		return image;
	}
		...
		try {
			...
			for (j1 = 0; j1 < i; j1++) {
				...
				for (k1 = -ai[j1]; k1 < 0; k1++) {
					 if(GameGraphics.ANTI_ALIASING.toggled()) {
						int x1 = k4 >> 16;
						int y1 = l4 >> 16;
						int x2 = x1 + 1;
						int y2 = y1 + 1;
    ...
    public void drawTooltip(int xPos, int yPos, Sprite hover, String[] toolTipText,String[] toolTipText1, int  colour) {
		if ((Client.instance.mouseX >= xPos) && (Client.instance.mouseX <= (xPos + myWidth)) && (Client.instance.mouseY >= yPos) && (Client.instance.mouseY <= (yPos + myHeight))) {
			Client.instance.drawTooltip(Client.instance.mouseX + 11, Client.instance.mouseY - 20, toolTipText,toolTipText1, 0, colour);
		}
	}
	
	public void drawTooltip(int xPos, int yPos, Sprite hover, String[] toolTipText, int  colour) {
		if ((Client.instance.mouseX >= xPos) && (Client.instance.mouseX <= (xPos + myWidth)) && (Client.instance.mouseY >= yPos) && (Client.instance.mouseY <= (yPos + myHeight))) {
			Client.instance.drawTooltip(Client.instance.mouseX + 11, Client.instance.mouseY - 20, toolTipText, 0, colour);
		}
	}
    ...
    public void method355(int ai[], int i, byte abyte0[], int j, int ai1[], int k, int l, int i1, int j1, int k1) {
        int l1 = -(i >> 2);
        i = -(i & 3);
