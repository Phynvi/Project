...
public final class CustomFont {
	...
	public void drawHoverString(String str, int x, int y, int color, int hoverColor, boolean center, boolean shaded,
		...
		if (Client.instance.mouseX >= x - (center ? stringWidth(str) / 2 : 0)
				&& Client.instance.mouseX <= x + stringWidth(str) - (center ? stringWidth(str) / 2 : 0)) {
			if (Client.instance.mouseY >= y - stringHeight(str) && Client.instance.mouseY <= y) {
				colorToUse = hoverColor;

				if (underline) {
					Rasterizer2D.drawHorizontalLine(x - 1 - (center ? stringWidth(str) / 2 : 0) + 1, y + 2 + 1,
							stringWidth(str) + 2, 0);
					Rasterizer2D.drawHorizontalLine(x - 1 - (center ? stringWidth(str) / 2 : 0), y + 2,
							stringWidth(str) + 2, hoverColor);
				}
			}
		}
		if (center)
	...
	private void drawPixels(final byte[] pixels, final int width, final int height, int xPos, int yPos,
		...
		int w = width;
		int h = height;
		int tmp = Rasterizer2D.leftX;
		if (xPos < tmp) {
			x += tmp - xPos;
			w -= tmp - xPos;
			xPos = tmp;
		}
		tmp = Rasterizer2D.topY;
		if (yPos < tmp) {
			y += tmp - yPos;
			h -= tmp - yPos;
			yPos = tmp;
		}
		tmp = Rasterizer2D.bottomX;
		if (xPos + w > tmp)
			w = tmp - xPos;

		tmp = Rasterizer2D.bottomY;
		if (yPos + h > tmp)
			h = tmp - yPos;
		...
		for (int i = 0; i != h; ++i) {
			for (int i1 = 0; i1 != w; ++i1)
				dst[globalIndex + i1] = blend(dst[globalIndex + i1], color,
						bind(pixels[localIndex + i1] & 0xff, color >>> 24)) & 0xffffff;
