package com.overload.draw.skillorbs;
...
public class SkillOrb {
    ...
    public void draw(int x, int y) {
        ...
        Rasterizer2D.drawFilledCircle(x + 24, y + 24, 20, 0x504a41, alpha < 180 ? alpha : 180);
        ...
        icon.drawAdvancedSprite(x + 25 - icon.myWidth / 2, 24 - icon.myHeight / 2 + y, alpha);
    }
    ...
    public Shape createRing(int x, int y, int angle) {
        Shape sector = Rasterizer2D.createSector(x + 2, y + 1, 45, angle);
        Shape innerCircle = Rasterizer2D.createCircle(x + 7, y + 6, 35);
    ...
    public void drawRing(Shape ring, Color colour) {
    	SkillOrbs.g2d.setRenderingHint(RenderingHints.KEY_ANTIALIASING,RenderingHints.VALUE_ANTIALIAS_ON);
    ...
    private Color getColour() {
        return currentLevel() >= 99 ? new Color(34,139,34, alpha) : new Color(232, 232, 232, alpha);
    }
