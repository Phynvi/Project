...
public class Rasterizer2D extends Cacheable {
    ...
    public static void draw_arc(int x, int y, int width, int height,
			int stroke, int start, double d, Color color, int alpha, int closure,
			boolean fill) {
		Graphics2D graphics = Sprite.createGraphics(Rasterizer2D.pixels,
				Rasterizer2D.width, Rasterizer2D.height);
		graphics.setColor(color);
		RenderingHints render = new RenderingHints(
				RenderingHints.KEY_ANTIALIASING,
				RenderingHints.VALUE_ANTIALIAS_ON);
		render.put(RenderingHints.KEY_STROKE_CONTROL,
				RenderingHints.VALUE_STROKE_PURE);// fix the 'jittering'
		...
		Arc2D.Double arc = new Arc2D.Double(x + stroke, y + stroke, width,
				height, start, d, closure);
		if (fill) {
			graphics.fill(arc);
		} else {
			graphics.draw(arc);
    ...
    public static void drawRoundedRectangle(int x, int y, int width, int height, int color, int alpha, boolean filled, boolean shadowed) {
		...
		if (alpha == -1) {
			if (filled) {
				method339(y + 1, color, width - 4, x + 2);//method339
				method339(y + height - 2, color, width - 4, x + 2);//method339
				drawPixels(height - 4, y + 2, x + 1, color, width - 2);//method336
			}
			method339(y, color, width - 4, x + 2);//method339
			method339(y + height - 1, color, width - 4, x + 2);//method339
	...
	public static void draw_arc(int x, int y, int width, int height,
			int stroke, int start, int sweep, int color, int alpha,
			int closure, boolean fill) {
		Graphics2D graphics = Sprite.createGraphics(Rasterizer2D.pixels,
				Rasterizer2D.width, Rasterizer2D.height);
		graphics.setColor(new Color((color >> 16 & 0xff), (color >> 8 & 0xff),
				(color & 0xff), ((alpha >= 256 || alpha < 0) ? 255 : alpha)));
		RenderingHints render = new RenderingHints(
				RenderingHints.KEY_ANTIALIASING,
				RenderingHints.VALUE_ANTIALIAS_ON);
		render.put(RenderingHints.KEY_STROKE_CONTROL,
