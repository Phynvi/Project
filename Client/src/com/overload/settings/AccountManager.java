...
public class AccountManager {
	 ...
	 */
	private static File saveAccountFile = new File(Signlink.getDataDirectory() + "/accounts.dat");
	...
	public static void loadAccount() {
		...
		try {
			...
			for (int index = 0; index < fileSize; index++) {
				...
				String profileurl = input.readUTF();
				AccountData account = new AccountData(rank, mode, username, password,profileurl);
				accounts.add(account);
			}
			input.close();
		} catch(Exception e) {
