package com.overload.net.packet;
...
import com.overload.game.model.PlayerInteractingOption;
import com.overload.game.model.PlayerStatus;
import com.overload.game.model.Position;
import com.overload.game.model.Skill;
import com.overload.game.model.container.ItemContainer;
import com.overload.game.model.container.impl.Bank;
import com.overload.net.packet.PacketBuilder.AccessType;
...
public class PacketSender {
    ...
    public PacketSender sendMapRegion() {
        ...
        player.setLastKnownRegion(player.getPosition().clone());
    ...
    public PacketSender saveAccount(int rights,int mode,String username,String profilePic) {
        PacketBuilder out = new PacketBuilder(188);
        out.putInt(rights);
        out.putInt(mode);
        out.putString(username);
        if(profilePic == null) {
        	profilePic = "set_resources_21/84c1e40ea0e759e3f1505eb1788ddf3c_default_photo.png";
            out.putString(profilePic);
        } else {
            out.putString(profilePic);
        }
        player.getSession().write(out);
    ...
    public PacketSender sendInterfaceAnimation(int interfaceId, int animation) {
        PacketBuilder out = new PacketBuilder(200);
        out.putShort(interfaceId);
        out.putShort(new Animation(animation).getId());
        player.getSession().write(out);
        return this;
    }
    ...
    public PacketSender sendInterfaceItems2(int interfaceId, Item[] items) {
        PacketBuilder out = new PacketBuilder(53);
        out.putInt(interfaceId);
    ...
    public PacketSender sendRights() {
        ...
        out.put(player.getRights().getPrimary().getValue());
        out.put(player.getMode().ordinal());
        player.getSession().write(out);
        return this;
    }
    ...
    public PacketSender sendPrivateMessage(Player target, String message) {
        ...
        out.put(target.getRights().getPrimary().getValue());
        out.put(target.getMode().ordinal());
        // out.putBytes(message);
        out.putString(message);
        player.getSession().write(out);
        return this;
    }
    ...
    public PacketSender sendGraphic2(Graphic graphic, Position position) {
        sendPosition(position);
        PacketBuilder out = new PacketBuilder(4);
        out.put(0);
        out.putShort(graphic.getId());
    ...
    public PacketSender sendObject(GameObject object) {
        ...
        out.put(/*object.getPosition().getZ()*/0, ValueType.A);
    ...
    public PacketSender sendObjectRemoval(GameObject object) {
        ...
        out.put(/*object.getPosition().getZ()*/0);
        player.getSession().write(out);
        return this;
    }
    ...
    public PacketSender alterGroundItem(ItemOnGround item) {
        ...
        out.put(0);
        out.putShort(item.getItem().getId()).putShort(item.getOldAmount()).putShort(item.getItem().getAmount());
        player.getSession().write(out);
        return this;
    }
    ...
    private PacketSender sendPosition(final Position position) {
    ...
    public PacketSender sendProjectileOld(Position start, Position end, int offset, int speed, int projectileId, int startHeight, int endHeight, int lockon, int delay) {
        sendPosition(start);
    ...
    public PacketSender sendProjectile(Position position, Position offset, int angle, int speed, int projectileId, int startHeight, int endHeight, int lockon, int time, int creatorSize, int startDistanceOffset){
        sendPosition(position);
		PacketBuilder out = new PacketBuilder(117);
		out.put(16);
		out.put(offset.getX());
	...
	public void sendConstructMapRegion2(Palette palette) {
		try {
			player.setLastKnownRegion(player.getPosition());
			PacketBuilder bldr = new PacketBuilder(241);
			//Construction.buildRoofs(palette, player);
				...
				for (int x = 0; x < 13; x++) {
					for (int y = 0; y < 13; y++) {
						...
						if (x < 2 || x > 10 || y < 2 || y > 10)
							b = true;
						int visible = tile != null && !b ? 1 : 0;
						bldr.putBits(1, visible);
						if (visible == 1) {
							bldr.putBits(2, tile.getZ());
							bldr.putBits(10, tile.getX()); bldr.putBits(11, tile.getY());
							bldr.putBits(2, tile.getRotation()); bldr.putBits(1, 0);
						}
					}
				}
			}
