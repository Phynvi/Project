package com.overload.game.content.minigames.barrows;
...
public class Barrows {
    ...
    public static final Position ENTRANCE = new Position(3565, 3306);
    public static final int KILLCOUNTER_INTERFACE_ID = 4535;
    public static final int KILLCOUNTER_FRAME_ID = 4536;
    public static final Position CHEST_ENTRANCE = new Position(3551, 9691);
    private static final Position BOSS_SPAWN = new Position(3550, 9694);
    private static final int CHEST_OBJECT_ID = 20723;
    ...
    public static boolean handleObject(Player player, int object) {
        ...
        if (brother.isPresent()) {
            ...
            if (player.getBarrowsCrypt() == object) {
                DialogueHandler.sendOptions(player, "Enter the tunnels?", new String[] {"Yes", "No"});
                player.setDialogueState(1);
                player.setNpcTalking(-10);
                player.setDialogueClickType(DialogueHandler.GLOBAL_OBJECT_CLICK);
                player.setDialogueClickId(player.getBarrowsCrypt());
                return true;
            }
            ...
            if (getKillcount(player) >= 5) {
                ...
                if (boss.isPresent()) {
                    ...
                    if (player.getKilledBrothers()[boss.get().ordinal()]) {
                        ...
                        player.sendMessage("@or3@You've looted a total of " + player.getBarrowsChestsLooted() + " chests.");
                        reset(player);
                    } else {
    ...
    public static void brotherSpawn(Player player, Brother brother, Position pos) {
    ...
    public static void brotherDespawn(Player player) {
        if (player.getCurrentBrother() != null) {
            if (player.getCurrentBrother().isRegistered() && !player.getCurrentBrother().isDying()) {
                World.getRemoveNPCQueue().add(player.getCurrentBrother());
    ...
    private static Optional<Brother> getDiggingLocation(Player player) {
        return Arrays.stream(Brother.values()).filter(x -> x.getBoundary().inside(player.getPosition())).findFirst();
    }
    ...
    public enum Brother {
        AHRIM_THE_BLIGHTED(1672, 20770,
                new Position(3557, 9701, 3),
                new Boundary(3562, 3568, 3285, 3292),
                new Position(3557, 9703, 3),
                20667,
                new Position(3565, 3288, 0)),
        DHAROK_THE_WRETCHED(1673, 20720,
                new Position(3553, 9716, 3),
                new Boundary(3572, 3578, 3294, 3301),
                new Position(3556, 9718, 3),
                20668,
                new Position(3574, 3297, 0)),
        GUTHAN_THE_INFESTED(1674, 20722,
                new Position(3540, 9705, 3),
                new Boundary(3574, 3584, 3279, 3285),
                new Position(3534, 9704, 3),
                20669,
                new Position(3577, 3282, 0)),
        KARIL_THE_TAINTED(1675, 20771,
                new Position(3549, 9685, 3),
                new Boundary(3564, 3568, 3273, 3278),
                new Position(3546, 9684, 3),
                20670,
                new Position(3566, 3275, 0)),
        TORAG_THE_CORRUPTED(1676, 20721,
                new Position(3568, 9688, 3),
                new Boundary(3550, 3556, 3280, 3284),
                new Position(3568, 9683, 3),
                20671,
                new Position(3554, 3282, 0)),
        VERAC_THE_DEFILED(1677, 20772,
                new Position(3575, 9708, 3),
                new Boundary(3553, 3560, 3294, 3301),
                new Position(3578, 9706, 3),
                20672,
                new Position(3557, 3297, 0));

        private int npcId, coffinId, stairs;

        private Position spawn, digSpawn, stairSpawn;
        private Boundary boundary;

        private Brother(final int npcId, final int coffin, final Position brotherSpawn, final Boundary boundary,
                        final Position digSpawn, final int stairs, final Position stairSpawn) {
            this.npcId = npcId;
