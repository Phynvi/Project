package com.overload.game.content.presets;
...
import com.overload.game.model.syntax.impl.ChangePresetName;
import com.overload.game.model.syntax.impl.CreatePreset;
...
public class Presetables {
	 ...
	 */
	private static final int INTERFACE_ID = 12000;
	...
	public static void open(Player player, Presetable preset) {
		if (preset != null) {
			...
			// Send name
			player.getPacketSender().sendString(12002, "Presets - " + preset.getName());

			// Send stats
			player.getPacketSender().sendString(12007, Integer.toString(preset.getStats()[3])); // Hitpoints
			player.getPacketSender().sendString(12008, Integer.toString(preset.getStats()[0])); // Attack
			player.getPacketSender().sendString(12009, Integer.toString(preset.getStats()[2])); // Strength
			player.getPacketSender().sendString(12010, Integer.toString(preset.getStats()[1])); // Defence
			player.getPacketSender().sendString(12011, Integer.toString(preset.getStats()[4])); // Ranged
			player.getPacketSender().sendString(12012, Integer.toString(preset.getStats()[5])); // Prayer
			player.getPacketSender().sendString(12013, Integer.toString(preset.getStats()[6])); // Magic

			// Send spellbook
			player.getPacketSender().sendString(12014,
					"@yel@" + Misc.formatText(preset.getSpellbook().name().toLowerCase()));
		} else {

			// Reset name
			player.getPacketSender().sendString(12002, "Presets");

			// Reset stats
			for (int i = 0; i <= 6; i++) {
				player.getPacketSender().sendString(12007 + i, "");
			}
		...
		for (int i = 0; i < 28; i++) {
			...
			if (item != null) {
				player.getPacketSender().sendItemOnInterface(12015 + i, item.getId(), item.getAmount());
			} else {
				player.getPacketSender().sendItemOnInterface(12015 + i, -1, 1);
			}
		}

		// Send equipment
		for (int i = 0; i < 14; i++) {
			player.getPacketSender().sendItemOnInterface(12044 + i, -1, 1);
		}
		if (preset != null) {
			Arrays.stream(preset.getEquipment()).filter(t -> !Objects.isNull(t) && t.isValid())
					.forEach(t -> player.getPacketSender().sendItemOnInterface(
							12044 + t.getDefinition().getEquipmentType().getSlot(), t.getId(), t.getAmount()));
		}

		// Send all available global presets
		for (int i = 0; i < MAX_PRESETS; i++) {
			player.getPacketSender().sendString(12070 + i,
					GLOBAL_PRESETS[i] == null ? "Empty" : GLOBAL_PRESETS[i].getName());
		}

		// Send all available player presets
		for (int i = 0; i < MAX_PRESETS; i++) {
			player.getPacketSender().sendString(12082 + i,
					player.getPresets()[i] == null ? "Empty" : player.getPresets()[i].getName());
		}

		// Send on death toggle
		player.getPacketSender().sendConfigByte(987, player.isOpenPresetsOnDeath() ? 0 : 1);
	...
	private static void load(Player player, final Presetable preset) {
		...
		player.getPacketSender().sendConfigByte(709, PrayerHandler.canUse(player, PrayerData.PRESERVE, false) ? 1 : 0);
		player.getPacketSender().sendConfigByte(711, PrayerHandler.canUse(player, PrayerData.RIGOUR, false) ? 1 : 0);
		player.getPacketSender().sendConfigByte(713, PrayerHandler.canUse(player, PrayerData.AUGURY, false) ? 1 : 0);
	...
	public static boolean handleButton(Player player, int button) {
		...
		if (button >= 12070 && button <= 12079) {
			final int index = button - 12070;
		...
		if (button >= 12082 && button <= 12091) {
			final int index = button - 12082;
