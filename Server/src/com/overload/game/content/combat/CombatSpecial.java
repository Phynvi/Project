...
import com.overload.game.content.combat.method.impl.specials.*;
...
public enum CombatSpecial {
	...
	// Ranged
	MAGIC_SHORTBOW(new int[] { 861 }, 55, 1.03, 1.2, new MagicShortbowCombatMethod(), WeaponInterface.SHORTBOW), DARK_BOW(new int[] { 11235 }, 55, 1.29, 1.24, new DarkBowCombatMethod(),
	...
	public static void updateBar(Player player) {
		...
		player.getPacketSender().sendString(
				player.getCombat().getWeapon().getSpecialMeter(),
	...
	public static void activate(Player player) {
		...
		if (player.isSpecialActivated()) {
			...
			if (spec == CombatSpecial.GRANITE_MAUL) {
				...
				Character target = player.getCombat().getTarget();
				if (target != null && CombatFactory.getMethod(player).getCombatType() == CombatType.MELEE) {
