...
public class DragonCombatMethod implements CombatMethod {
    ...
    public void prepareAttack(Character character, Character target) {
        ...
        switch (currentAttackType) {
            case MAGIC:
                ...
                character.performGraphic(new Graphic(1));
                break;
            case MELEE:
        ...
        if (target.isPlayer()) {
            ...
            if (currentAttackType == CombatType.MAGIC) {
                if (PrayerHandler.isActivated(p, PrayerHandler.PROTECT_FROM_MAGIC)
                    ...
                    target.getAsPlayer().getPacketSender().sendMessage("@or3@You're protected against the dragonfire breath.");
                ...
                p.getAsPlayer().getPacketSender().sendMessage("@or3@The dragonfire burns you.");
