...
public class Firemaking extends DefaultSkillable {
    ...
    public void finishedCycle(Player player) {
        ...
        if (bonfire.isPresent() || groundLog.isPresent() && ItemOnGroundManager.exists(groundLog.get())) {
            ...
            if (!bonfire.isPresent()) {
                ...
                //Create fire..
                TaskManager.submit(new TimedObjectSpawnTask(new GameObject(ObjectIdentifiers.FIRE_5, pos), log.getRespawnTimer(), Optional.of(new Action() {
                    @Override
                    public void execute() {
    ...
    public int cyclesRequired(Player player) {
        ...
        int cycles = log.getCycles() + Misc.getRandom(3);
    ...
    public boolean hasRequirements(Player player) {
        ...
        if (bonfire.isPresent()) {
            if (!ObjectManager.exists(ObjectIdentifiers.FIRE_5, bonfire.get().getPosition())) {
                return false;
            }
        } else {
            ...
            if (/*ClippedRegionManager.getObject(player.getPosition()).isPresent()
                    ||*/ ObjectManager.exists(player.getPosition())) {
