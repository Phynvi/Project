...
public class ItemCreationSkillable extends DefaultSkillable {
    ...
    public ItemCreationSkillable(List<RequiredItem> requiredItems, Item product, int amount, Optional<AnimationLoop> animLoop, int requiredLevel, int experience, Skill skill) {
    ...
    public boolean hasRequirements(Player player) {
        ...
        if (player.getSkillManager().getCurrentLevel(skill) < requiredLevel) {
            player.getPacketSender().sendMessage("You need a " + skill.getName() + " level of at least " + Integer.toString(requiredLevel) + " to do this.");
            return false;
        }
        ...
        for (RequiredItem item : requiredItems) {
            if (!player.getInventory().contains(item.getItem())) {
                ...
                player.getPacketSender().sendMessage("You " + (!hasItems ? "also need" : "need") + " " + prefix + " " + item.getItem().getDefinition().getName() + ".");
                hasItems = false;
            }
        }
        if (!hasItems) {
