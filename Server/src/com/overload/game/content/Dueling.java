...
public class Dueling {
    ...
    public void initiateDuel() {
        ...
        player.getPacketSender().sendConfigByte(RULES_CONFIG_ID, 0);
    ...
    private void checkRule(DuelRule rule) {
        ...
        if (state == DuelState.DUEL_SCREEN || state == DuelState.ACCEPTED_DUEL_SCREEN) {
            ...
            player.getPacketSender().sendConfigByte(RULES_CONFIG_ID, configValue);
            interact.getPacketSender().sendConfigByte(RULES_CONFIG_ID, configValue);
    ...
    private void startDuel(Position telePos) {
        ...
        // Send interact hints
        player.getPacketSender().sendPositionalHint(interact.getPosition().clone(), 10);
    ...
    public void duelLost() {
        ...
        if (validate(player, interact, null, new DuelState[]{DuelState.STARTING_DUEL, DuelState.IN_DUEL})) {
            ...
            // Move players home
            Position spawn = new Position(3366, 3266);
