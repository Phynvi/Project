...
public class D_3218 extends Dialogue {
    @Override
	...
	public boolean handleThree(Player player, NPC npc) {
        ...
        switch (handle) {
	        case 1:
	    		ShopManager.open(player, ShopIdentifiers.MAGIC_ROBES_EXCHANGE);
                DialogueHandler.close(player, true);
	            return true;
	        case 2:
	    		ShopManager.open(player, ShopIdentifiers.MAGIC_STAFF_EXCHANGE);
                DialogueHandler.close(player, true);
	            return true;
	        case 3:
	    		ShopManager.open(player, ShopIdentifiers.RUNES_EXCHANGE);
                DialogueHandler.close(player, true);
	            return true;
            default:
                return false;
