...
public class PrayerHandler {
    ...
    public static void activatePrayer(Character character, final int prayerId) {
        ...
        if (character.isPlayer()) {
            ...
            if (player.getSkillManager().getCurrentLevel(Skill.PRAYER) <= 0) {
                player.getPacketSender().sendConfigByte(pd.configId, 0);
        ...
        if (character.isPlayer()) {
            ...
            p.getPacketSender().sendConfigByte(pd.configId, 1);
            if (!hasNoPrayerOn(p) && !p.isDrainingPrayer()) {
    			if (!p.isDebug()) {
                    startDrain(p);
                }
            }
    ...
    public static boolean canUse(Player player, PrayerData prayer, boolean msg) {
        if (player.getSkillManager().getMaxLevel(Skill.PRAYER) < (prayer.requirement)) {
            if (msg) {
                player.getPacketSender().sendConfigByte(prayer.configId, 0);
    ...
    public static void deactivatePrayer(Character c, int prayerId) {
        ...
        if (c.isPlayer()) {
            ...
            p.getPacketSender().sendConfigByte(pd.configId, 0);
            if (pd.hint != -1) {
    ...
    private static final double getDrain(Player player) {
        ...
        if (toRemove > 0) {
        	double remove2 = 1 + (0.05 * player.getBonusManager().getOtherBonus()[BonusManager.PRAYER]);
    		toRemove /= remove2;
        }
        return toRemove;
    }
