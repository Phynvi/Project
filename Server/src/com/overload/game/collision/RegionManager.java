package com.overload.game.collision;
...
import com.overload.util.CompressionUtil;
...
public class RegionManager {
    ...
    public static Optional<Region> getRegion(int x, int y) {
        loadMapFiles(x, y); // its txt files now whut? the gz are the packed txt files :P
        int regionX = x >> 3;
        int regionY = y >> 3;
    ...
    public static void addObjectClipping(GameObject object) {
        ...
        try {
        	...
        	System.out.println("Array Index Out Of Bounds: " + id + " at Position: " + x + ", " + y + ", " + height);
        }
        if (def == null) {
            return;
        }
    ...
    public static boolean canMove(Position pos) {
        return (!blockedNorthWest(pos) && !blockedNorth(pos) && !blockedWest(pos)) ? true :
        		!blockedNorth(pos) ? true :
        	   (!blockedNorthEast(pos) && !blockedNorth(pos) && !blockedEast(pos)) ? true :
        		!blockedWest(pos) ? true :
        		!blockedEast(pos) ? true :
        	   (!blockedSouthWest(pos) && !blockedSouth(pos) && !blockedWest(pos)) ? true :
        		!blockedSouth(pos) ? true :
    		   (!blockedSouthEast(pos) && !blockedSouth(pos) && !blockedEast(pos)) ? true : false;
    }

    public static boolean blockedProjectile(Position pos) {
        return (getClipping(pos.getX(), pos.getY(), pos.getZ()) & 0x20000) == 0;
    }
	...
	public static Palette getAsPalette(int x, int y) {
		Palette palette = new Palette();
		PaletteTile tile = null;

        Optional<Region> optionalRegion = getRegion(x, y);
        if (optionalRegion.isPresent()) {
        	Region region = optionalRegion.get();
        	
        	int regionId = region.getRegionId();
        	
        	int originX = (regionId >> 8) << 6;
    		int originY = (regionId & 0xFF) << 6;
    		
			for (int tileIndexY = 0; tileIndexY <= 9; tileIndexY++) {
				for (int tileIndexX = 0; tileIndexX <= 9; tileIndexX++) {
    				tile = new PaletteTile(originX+((tileIndexX*8)-16), originY+((tileIndexY*8)-16));
    				palette.setTile(tileIndexX, tileIndexY, 0, tile);
    				//System.out.println("[X=" + tileIndexX + "] " + (originX+(tileIndexX*8)) + " [Y=" + tileIndexY + "]: " + (originY+(tileIndexY*8)));
    			}
    		}
	...
	public static Palette getBoundaryAsPalette(Boundary boundary) {
		Palette palette = new Palette();
		PaletteTile tile = null;

    	int originX = boundary.getX();
		int originY = boundary.getY();

		int max_x = (int) Math.ceil((boundary.getX2() - boundary.getX()) % 8);
		int max_y = (int) Math.ceil((boundary.getY2() - boundary.getY()) % 8);
		
		for (int tileIndexY = 0; tileIndexY <= max_y; tileIndexY++) {
			for (int tileIndexX = 0; tileIndexX <= max_x; tileIndexX++) {
				tile = new PaletteTile(originX+((tileIndexX*8)-16), originY+((tileIndexY*8)-16));
				palette.setTile(tileIndexX, tileIndexY, 0, tile);
				//System.out.println("[X=" + tileIndexX + "] " + (originX+(tileIndexX*8)) + " [Y=" + tileIndexY + "]: " + (originY+(tileIndexY*8)));
			}
	...
	public static Area getRegionAsArea(int x, int y) {
        ...
        if (optionalRegion.isPresent()) {
			...
			boundaries.add(new Boundary(_x, _x + 64, _y, _y + 64));

			System.out.println("[AREA]");
			System.out.println("X1: " + _x);
			System.out.println("X2: " + (_x + 64));
			System.out.println("Y1: " + _y);
			System.out.println("Y2: " + (_y + 64));
			...
			return new Area(boundaries) {
				...
				public void process(Character character) {
					...
					if (character.isPlayer()) {
						character.getAsPlayer().sendMessage("Howdy");
					}
				}
	...
	public static Position getPlayerPositionInRegion(Player player, int oldx, int oldy, int newx, int newy) {
        Optional<Region> optionalRegion = getRegion(oldx, oldy);
        if (optionalRegion.isPresent()) {
	    	...
	    	int oldOriginX = (regionId >> 8) << 6;
			int oldOriginY = (regionId & 0xFF) << 6;
			
			int relativeX = position.getX() - oldOriginX;
			int relativeY = position.getY() - oldOriginY;
			
			relativeX = (newx-38) + relativeX;
			relativeY = (newy-38) + relativeY;
			
			return new Position(relativeX, relativeY, player.getIndex() * 4);
        }
