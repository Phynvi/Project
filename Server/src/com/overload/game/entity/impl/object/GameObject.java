...
public class GameObject extends Entity {
    ...
    public GameObject(int id, Position position, int type, int face, Optional<Player> spawnedFor) {
        super(NodeType.OBJECT, position);
        this.id = id;
        this.type = type;
        this.face = face;
    ...
    public void performAnimation(int animation) {
        for (Player player : World.getPlayers()) {
            if (player == null)
                continue;
            if (player.getPosition().isWithinDistance(getPosition()))
                player.getPacketSender().sendObjectAnimation(this, new Animation(animation));
        }
    }
