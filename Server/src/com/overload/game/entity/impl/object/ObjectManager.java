package com.overload.game.entity.impl.object;
...
public class ObjectManager {
    ...
    public static void register(GameObject object, boolean playerUpdate) {
        ...
        for (; iterator.hasNext(); ) {
            ...
            if (o.getPosition().equals(object.getPosition()) && o.getType() == object.getType()) {
                iterator.remove();
            }
        }
            ...
            switch (type) {
                ...
                case DESPAWN:
                    for (Player player : World.getPlayers()) {
                        if (player == null)
                            continue;
    					if(object.getInstancedRegion().isPresent()) {
    						if(!player.getInstancedRegion().isPresent() 
    								|| player.getInstancedRegion().get() != object.getInstancedRegion().get()) {
    							continue;
    						}
    					}
                        if (player.getPosition().isWithinDistance(object.getPosition(), 64)) {
                            if (type == OperationType.SPAWN) {
    ...
    public static Optional<GameObject> get(Position position) {
        Iterator<GameObject> iterator = World.getObjects().iterator();
        for (; iterator.hasNext(); ) {
            GameObject object = iterator.next();
            if (object.getPosition().equals(position)) {
