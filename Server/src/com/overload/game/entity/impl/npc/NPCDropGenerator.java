...
import com.overload.game.definition.NpcDropDefinition.DropTable;
import com.overload.game.definition.NpcDropDefinition.NPCDrop;
import com.overload.game.definition.NpcDropDefinition.RDT;
...
import com.overload.util.RandomGen;
...
public class NPCDropGenerator {
     ...
     * @param npc
     */
    public static void start(Player player, NPC npc, Optional<Position> lootPos) {
        Optional<NpcDropDefinition> def = NpcDropDefinition.get(npc.getId());
        if (def.isPresent()) {
            NPCDropGenerator gen = new NPCDropGenerator(player, def.get());
            Position pos = lootPos.isPresent() ? lootPos.get() : npc.getPosition();
            pos.setZ(npc.getPosition().getZ());
            for (Item item : gen.getDropList()) {
            	if (!item.getDefinition().isStackable()) {
            		for (int i = 0; i < item.getAmount(); i++) {
            			ItemOnGroundManager.register(player, new Item(item.getId(), 1), pos);
            		}
            	} else {
        ...
        for (int i = 0; i < rolls; i++) {
            ...
            if (!table.isPresent()) {
                ...
                if ((table = getDropTable(chance)).isPresent()) {
                    ...
                    NPCDrop npcDrop = dropTableItems.get()[Misc.random(dropTableItems.get().length)-1];//random.get().nextInt(dropTableItems.get().length)-1];
