package com.overload.game.entity.impl.grounditem;
...
import com.overload.game.task.impl.GroundItemRespawnTask;
...
public class ItemOnGroundManager {
	...
	public static void perform(Player player, ItemOnGround item, OperationType type) {
		...
		if (item.getPosition().getZ() != player.getPosition().getZ())
			return;
		if (item.getPosition().getDistance(player.getPosition()) > 64)
			return;
		switch (type) {
		case ALTER:
	...
	public static ItemOnGround register(Player player, Item item, Position position) {
		ItemOnGround i = new ItemOnGround(State.SEEN_BY_PLAYER, Optional.of(player.getUsername()), position, item, true,-1);
		register(i);
		HandleLogs.executeLog(player, item.getDefinition().getName() + " [" + position.getX() + "," + position.getY() + "," + position.getZ() + "]", HandleLogs.LogType.DROP);
		return i;
	}
	...
	public static void registerNonGlobal(Player player, Item item, Position position) {
		register(new ItemOnGround(State.SEEN_BY_PLAYER, Optional.of(player.getUsername()), position, item, false, -1));
	}
	...
	public static void registerGlobal(Player player, Item item) {
		register(new ItemOnGround(State.SEEN_BY_EVERYONE, Optional.of(player.getUsername()), player.getPosition().clone(), item, false, -1));
	}
	 ...
	 * @return
	 */
	public static Optional<ItemOnGround> getGroundItem(Optional<String> owner, int id, Position position) {
		Iterator<ItemOnGround> iterator = World.getItems().iterator();
		for (; iterator.hasNext();) {
			ItemOnGround item = iterator.next();
