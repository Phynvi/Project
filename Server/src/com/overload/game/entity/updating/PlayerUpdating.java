package com.overload.game.entity.updating;
...
import com.overload.net.packet.ByteOrder;
import com.overload.net.packet.PacketBuilder;
import com.overload.net.packet.ValueType;
...
public class PlayerUpdating {
	...
	public static void update(final Player player, boolean forceUpdate) {
		...
		appendUpdates(player, update, player, forceUpdate, true);
		...
		for (Iterator<Player> playerIterator = player.getLocalPlayers().iterator(); playerIterator.hasNext();) {
			...
			if (World.getPlayers().get(otherPlayer.getIndex()) != null
					&& otherPlayer.getPosition().isWithinDistance(player.getPosition()) && !otherPlayer.isNeedsPlacement()) {
	...
	private static void updateChat(PacketBuilder builder, Player target) {
		...
		builder.put(target.getRights().getPrimary().getValue());
		builder.put(target.getMode().ordinal());
		builder.putString(message.getText());
		/*
		 * builder.put(bytes.length, ValueType.C); for (int ptr = bytes.length - 1; ptr
		 * >= 0; ptr--) { builder.put(bytes[ptr]); }
		 */
	}
	...
	private static void updateAppearance(Player player, PacketBuilder out, Player target) {
		...
		properties.put(target.getRights().getPrimary().getValue());
		properties.put(target.getMode().ordinal());
