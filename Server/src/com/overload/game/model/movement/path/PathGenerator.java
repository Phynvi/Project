package com.overload.game.model.movement.path;
...
public class PathGenerator {
    ...
    public static Position getCombatPath(Character character, Position targetPos, int targetSize) {
        ...
        int followX = targetPos.getX();
        int followY = targetPos.getY();
        ...
        for (int height = 0; height < 4; height++) {
            for (int i = 0; i < loop - (height == 0 ? 1 : 0); i++) {
                if (height == 0) {
                    x3++;
                } else if (height == 1) {
                    if (i == 0) {
    ...
    public static Position getBasicPath(Character character, Position targetPos) {
        ...
        for (int i = 0; i < nodes.length; i++) {
            if (isInteractionPathClear(nodes[i][0], nodes[i][1], z, x, y)) {
                ...
                if ((dist < bestDist) && (RegionManager.canMove(character.getPosition(), NON_DIAGONAL_DIRECTIONS[i]))) {
                    bestDist = dist;
                    bestX = nodes[i][0];
                    bestY = nodes[i][1];
                }
    ...
    public static boolean isInteractionPathClear(final int x0, final int y0, final int characterZ, final int followX, final int followY) {
        ...
        while (true) {
            ...
            if (!RegionManager.canProjectileMove(pX, pY, followX, followY, characterZ, 1, 1)) {
                return false;
            }
