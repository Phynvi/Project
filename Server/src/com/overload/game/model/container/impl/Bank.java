package com.overload.game.model.container.impl;
...
import com.overload.game.model.container.StackType;
import com.overload.game.model.equipment.BonusManager;
import com.overload.game.model.syntax.impl.BankPin;
import com.overload.game.model.syntax.impl.SearchBank;
...
public class Bank extends ItemContainer {
    ...
    public static void deposit(Player player, int item, int slot, int amount) {
        if (player.getStatus() == PlayerStatus.BANKING && player.getInterfaceId() == 5292) {
            ...
            if (player.getBank(tab).getAmount(item) + amount > Integer.MAX_VALUE || player.getBank(tab).getAmount(item) + amount <= 0) {
            	amount = Integer.MAX_VALUE - player.getBank(tab).getAmount(item);
            }
            ...
            if (player.getStatus() == PlayerStatus.BANKING) {
                ...
                switch (button) {
                    ...
                    case 50010:
                        ...
                        player.getPacketSender().sendConfigByte(118, player.isPlaceholders() ? 1 : 0);
                        player.getPacketSender().sendMessage("Placeholders are now " + (player.isPlaceholders() ? "enabled" : "disabled") + ".");
                     break;
                    case 50009:
                    	player.setEnterSyntax(new BankPin());
                    	if(player.getPin() == "") {
                    		player.getPacketSender().sendEnterInputPrompt("Create a Bank Pin.");
                    	} else {
                    		player.getPacketSender().sendEnterInputPrompt("Change your Bank Pin.");
                    	}
    ...
    public Bank open() {
    	if(!getPlayer().hasEnteredPin()) {
    		getPlayer().getPacketSender().sendEnterInputPrompt("Enter your bank pin.");
    		getPlayer().setEnterSyntax(new BankPin());
    	}
        ...
        // Send configs
        getPlayer().getPacketSender().sendConfigByte(115, getPlayer().withdrawAsNote() ? 1 : 0)
                .sendConfigByte(304, getPlayer().insertMode() ? 1 : 0)
                .sendConfigByte(117, getPlayer().isSearchingBank() ? 1 : 0)
                .sendConfigByte(118, getPlayer().isPlaceholders() ? 1 : 0).sendInterfaceSet(5292, 5063);
    ...
    public Bank refreshItems() {
        ...
        if (getPlayer().isSearchingBank()) {
            getPlayer().getPacketSender().sendString(5383, "Results for " + getPlayer().getSearchSyntax() + "..")
                    .sendConfigByte(117, 117);
        } else {
            getPlayer().getPacketSender().sendString(5383, "Bank of " + GameConstants.NAME).sendConfigByte(117, 0);
        }
    ...
    public Bank switchItem(ItemContainer to, Item item, int slot, boolean sort, boolean refresh) {
        ...
        if (to instanceof Inventory) {
            boolean withdrawAsNote = getPlayer().withdrawAsNote() && def != null && def.isNoted() && item.getDefinition() != null && def.getName().equalsIgnoreCase(item.getDefinition().getName());
            ...
            if (to.getAmount(checkId) + item.getAmount() > Integer.MAX_VALUE || to.getAmount(checkId) + item.getAmount() <= 0) {
            	item.setAmount(Integer.MAX_VALUE - to.getAmount(checkId));
            }
        } else {
        	if (to.getAmount(item.getId()) + item.getAmount() > Integer.MAX_VALUE || to.getAmount(item.getId()) + item.getAmount() <= 0) {
